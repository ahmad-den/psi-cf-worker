...
var CACHE_TTL = 1e3 * 60;
var REQUIRED_HEADERS = {
  "x-hosted-by": "BigScoots",
  "x-bigscoots-cache-plan": "Performance+"
};
var DEFAULT_STRATEGY = "mobile";
var TIMEOUTS = {
  HEADER_VALIDATION: 1e4,
  // 10 seconds
  PSI_API: 3e4
  // 30 seconds
};
var CACHE_SETTINGS = {
  HEADER_VALIDATION_TTL: 300
  // 5 minutes
};
...
__name(parseRequestParams, "parseRequestParams");

// src/services/header-validator.ts
async function validateTargetHeaders(targetUrl) {
  try {
    ...
    console.error("Header validation error:", error3);
    return false;
  }
}
__name(validateTargetHeaders, "validateTargetHeaders");
...
async function getCachedResponse(kv, params) {
  try {
    ...
    console.error("Cache retrieval error:", error3);
    return null;
  }
}
__name(getCachedResponse, "getCachedResponse");
async function cacheResponse(kv, params, data) {
  try {
    const cacheKey = generateCacheKey(params);
    const cacheData = {
      data,
    ...
    console.error("Cache storage error:", error3);
  }
}
...
async function fetchPSIData(params, apiKey) {
  try {
    ...
    const domain2 = extractDomainFromUrl(params.url);
    const customUserAgent = `cwv-insights-${domain2}`;
    ...
    console.error("PSI fetch error:", error3);
    return null;
  }
}
__name(fetchPSIData, "fetchPSIData");
function buildPSIUrl(params, apiKey) {
...
__name(buildPSIUrl, "buildPSIUrl");
function extractDomainFromUrl(url) {
  try {
    const urlObj = new URL(url);
    return urlObj.hostname.replace(/^www\./, "");
  } catch (error3) {
    console.error("Error extracting domain:", error3);
    return "unknown";
  }
}
__name(extractDomainFromUrl, "extractDomainFromUrl");
function processUsingGoogleApproach(data) {
  ...
  const auditGroupMap = /* @__PURE__ */ new Map();
  auditRefs.forEach((auditRef) => {
    const group3 = auditRef.id.endsWith("-insight") ? "insights" : auditRef.group ?? "diagnostics";
    auditGroupMap.set(auditRef.id, group3);
  });
  const allGroups = /* @__PURE__ */ new Set();
  auditGroupMap.forEach((group3) => allGroups.add(group3));
  ...
  allGroups.forEach((group3) => {
    organizedAudits.failed[group3] = {};
    organizedAudits.warnings[group3] = {};
  ...
  for (const [auditId, audit] of Object.entries(audits)) {
    if (audit && typeof audit === "object" && "score" in audit) {
      ...
      if (!group3) {
        group3 = auditId.endsWith("-insight") ? "insights" : "diagnostics";
      }
      if (score === 0) {
        organizedAudits.failed[group3][auditId] = audit;
      } else if (score === 0.5) {
        organizedAudits.warnings[group3][auditId] = audit;
      } else if (score === 1) {
        organizedAudits.passed[group3][auditId] = audit;
      }
    }
  }
...
__name(processUsingGoogleApproach, "processUsingGoogleApproach");
function extractCoreMetrics(audits, auditGroupMap) {
  const coreMetricIds = {
...
__name(extractCoreMetrics, "extractCoreMetrics");
...
__name(getScoreCategory, "getScoreCategory");
function getMetricWeight(auditId) {
  const weights = {
...
__name(getMetricWeight, "getMetricWeight");
function getMetricThresholds(auditId) {
  const thresholds = {
...
__name(getMetricThresholds, "getMetricThresholds");

// src/utils/response-builder.ts
function createErrorResponse(message, status, errorType) {
  return new Response(
    JSON.stringify({
      status: "failed",
      message,
...
function createSuccessResponse(data, options) {
  ...
  return new Response(
    JSON.stringify({
      status: "success",
      message: data.fromCache ? "PageSpeed Insights data retrieved from cache successfully" : "PageSpeed Insights data fetched successfully",
      data: responseData,
      meta: {
...
async function handleRequest(request, env2, ctx) {
  try {
    ...
    let psiData;
    if (!bypassCache) {
      psiData = await getCachedResponse(env2.PSI_CACHE, params);
    }
    if (!psiData) {
      psiData = await fetchPSIData(params, env2.GOOGLE_PSI_API_KEY);
      if (!bypassCache && psiData) {
        ctx.waitUntil(cacheResponse(env2.PSI_CACHE, params, psiData));
      }
    }
    if (!psiData) {
      return createErrorResponse(
...
// .wrangler/tmp/bundle-UOaaPg/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UOaaPg/middleware-loader.entry.ts
